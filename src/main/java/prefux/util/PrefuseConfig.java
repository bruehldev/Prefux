...
/**
 * <p>Runtime configuration settings for the prefux framework. Maintains a set
 * of hardwired configuration settings that can be overridden by creating a
 * text properties file containing custom values. By default, prefux will
 * look for the file "prefux.conf" in the current working directory for
 ...
 * "prefux.config" System property (for example by using a -D flag at the
 ...
 * the standard Java default. The default prefux setting is 6.</li>
 * <li><code>data.io.worker.threadPriority</code> - the thread priority of
 * asynchronous database worker threads. The default prefux setting is 5
 ...
 * <li><code>util.logdir</code> - the directory in which to write prefux log
 ...
 * prefux log files. The default is "prefuse_log_%g.txt", where the %g
...
public class PrefuseConfig extends Properties {
    ...
    private PrefuseConfig() {
        ...
        String configFile;
        try {
            configFile = System.getProperty("prefux.config");
        } catch ( Exception e ) {
            // in applet mode, we could run afoul of the security manager
            configFile = null;
        }
        if ( configFile == null )
            configFile = "prefux.conf";
        try {
        ...
        if ( logdir != null ) {
            try {
                Logger logger = Logger.getLogger("prefux");
    ...
    /**
     * Get a prefux configuration property.
    ...
    /**
     * Get a prefux configuration property as an integer.
     * @param key the name of the property to lookup
    ...
    /**
     * Get a prefux configuration property as a long.
     * @param key the name of the property to lookup
     * @return the property value, or the minimum possible
    ...
    /**
     * Get a prefux configuration property as a float.
     * @param key the name of the property to lookup
    ...
    /**
     * Get a prefux configuration property as a double.
     * @param key the name of the property to lookup
    ...
    /**
     * Get a prefux configuration property as a boolean.
     * @param key the name of the property to lookup
    ...
    private void setDefaults() {        
        ...
        // prefux will only attempt to optimize filtering operations
